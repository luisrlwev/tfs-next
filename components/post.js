...
export default function Post({ post }) {
  const { featured_image_src, title, excerpt, categories_names, date, comments_count, slug } = post;
  return (
    <article>
      <div className="border rounded-lg hover:drop-shadow-md flex flex-col overflow-hidden relative bg-white w-full min-h-full">
        ...
        <div className="p-4 space-y-2 relative block altura-blog">
          <Link href={`/blog/${slug}`}><h2 className="text-tfs text-xl font-bold mb-4 hover">{title.rendered}</h2></Link>
          <p className="text-sm text-black pb-4">{truncateText(stripHtml(excerpt.rendered), 180)}</p>
          <Link href={`/blog/${slug}`} className="uppercase text-tfs font-bold hover">Leer más »</Link>
          {/* fix bottom section */}
        </div>
